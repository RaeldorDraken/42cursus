...
#include <map> // ? This is the container we are going to use
...
private:
  ...
  std::map<std::string, ASpell *> spells; // ? The container used in this module. map allow us to set pairs creating some sort of "key/value" associations using the std::string as key / the name of the spell and ASpell * as values for each position in the container
