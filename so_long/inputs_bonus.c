...
/*   Updated: 2022/05/10 12:41:24 by eros-gir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redraw_level(t_vars *vars, int tile_x, int tile_y, char *movstr)
{
	...
	if (check_tile(vars->level[tile_y / 64][tile_x / 64]) == 3)
		mlx_put_image_to_window(vars->mlx, vars->win, vars->spr[3],
			tile_x, tile_y);
	if (check_tile(vars->level[tile_y / 64][tile_x / 64]) == 5)
		mlx_put_image_to_window(vars->mlx, vars->win, vars->spr[5],
			tile_x, tile_y);
	mlx_string_put(vars->mlx, vars->win, 33, 33, 0x00000000, movstr);
...
int	render_frame(t_vars *vars)
{
	...
	movstr = ft_strjoin("Movimientos: ", ft_itoa(vars->movcount));
	tile_y = 0;
	vars->score = 0;
